...
export class JobCookSIGenerateDownloads extends JobCook<JobCookSIGenerateDownloadsParameters> {
    ...
    private async createSystemObjects(): Promise<H.IOResults> {
        ...
        if (!SOV)
            return this.logError(`JobCookSIGenerateDownloads.createSystemObjects unable to clone SystemObjectVersion for ${H.Helpers.JSONStringify(sceneSystemObject)}`);
    ...
    public static computeModelAutomationTagFromDownloadType(downloadType: string): string {
        ...
        if(!usage || !quality || !uvResolution) {
            LOG.error(`JobCookSIGenerateDownloads.computeModelAutomationTag unsupported downloadType: '${downloadType}' (${usage} | ${quality} | ${uvResolution})`,LOG.LS.eJOB);

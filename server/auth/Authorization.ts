...
export class Authorization {
    ...
    static async buildContext(idUser: number): Promise<AuthorizationContext> {
        ...
        //   unrestricted Projects in authorized Units + Projects where user is explicitly listed
        //   (the user list is additive for unrestricted projects, exclusive for restricted ones)
        const unrestrictedProjects = await DBAPI.Project.fetchUnrestrictedByUnits(authorizedUnitIds);
        const listedProjects = await DBAPI.Project.fetchWithUserAuthorization(idUser);

        const effectiveProjectIdSet = new Set([
            ...unrestrictedProjects.map(p => p.idProject),
            ...listedProjects.map(p => p.idProject)
        ]);
        const effectiveProjectIds = [...effectiveProjectIdSet];

        // Compute effective unit set:
        //   directly-assigned units + parent units of projects accessed via explicit user listing
        //   (covers both restricted projects and additive grants outside the user's units)

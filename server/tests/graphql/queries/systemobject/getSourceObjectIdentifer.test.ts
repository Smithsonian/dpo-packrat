...
const getSourceObjectIdentiferTest = (utils: TestSuiteUtils): void => {
    ...
    describe('Query: getSourceObjectIdentifer', () => {
                        ...
                        if (Subject) {
                            ...
                            if (SO) {
                                ...
                                const input: GetSourceObjectIdentiferInput = {
                                    idSystemObjects: [SO.idSystemObject]
                                };
                                const result: GetSourceObjectIdentiferResult = await graphQLApi.getSourceObjectIdentifer(input);
                                const [{ idSystemObject, identifier }] = result.sourceObjectIdentifiers;
                                expect(idSystemObject).toBe(SO.idSystemObject);
...
export default getSourceObjectIdentiferTest;

...
export default async function deleteMetadata(_: Parent, args: MutationDeleteMetadataArgs): Promise<DeleteMetadataResult> {
    ...
    if (!metadata) {
        RK.logError(RK.LogSection.eGQL,'delete metadata failed',`Unable to retrieve Metadata with idMetadata ${idMetadata}`,{},'GraphQL.SystemObject.Metadata');
    ...
    return withAuditTransaction(async () => {
        if (await metadata.delete()) {
            RK.logInfo(RK.LogSection.eGQL,'delete metadata',`Metadata deleted with idMetadata ${idMetadata}`,{},'GraphQL.SystemObject.Metadata');
            return { success: true };
        }
        RK.logError(RK.LogSection.eGQL,'delete metadata failed',`Error deleting Metadata with idMetadata ${idMetadata}`,{},'GraphQL.SystemObject.Metadata');
        return { success: false };
    });
}
...
const Vocabulary = {
    ...
    CaptureDataFile: async (parent: Parent, _: Args, context: Context): Promise<CaptureDataFile[] | null> => {
        ...
        return prisma.vocabulary.findOne({ where: { idVocabulary: Number.parseInt(idVocabulary) } }).CaptureDataFile();
    },
    Identifier: async (parent: Parent, _: Args, context: Context): Promise<Identifier[] | null> => {
        const { idVocabulary } = parent;
        const { prisma } = context;

        return prisma.vocabulary.findOne({ where: { idVocabulary: Number.parseInt(idVocabulary) } }).Identifier();
    },
    Metadata: async (parent: Parent, _: Args, context: Context): Promise<Metadata[] | null> => {
        const { idVocabulary } = parent;
        const { prisma } = context;

        return prisma.vocabulary.findOne({ where: { idVocabulary: Number.parseInt(idVocabulary) } }).Metadata();
    },
    ModelGeometryFile: async (parent: Parent, _: Args, context: Context): Promise<ModelGeometryFile[] | null> => {
        const { idVocabulary } = parent;
        const { prisma } = context;

        return prisma.vocabulary.findOne({ where: { idVocabulary: Number.parseInt(idVocabulary) } }).ModelGeometryFile();
    },
    ModelProcessingActionStep: async (parent: Parent, _: Args, context: Context): Promise<ModelProcessingActionStep[] | null> => {
        const { idVocabulary } = parent;
        const { prisma } = context;

        return prisma.vocabulary.findOne({ where: { idVocabulary: Number.parseInt(idVocabulary) } }).ModelProcessingActionStep();
    },
    ModelUVMapChannel: async (parent: Parent, _: Args, context: Context): Promise<ModelUVMapChannel[] | null> => {
        const { idVocabulary } = parent;
        const { prisma } = context;

        return prisma.vocabulary.findOne({ where: { idVocabulary: Number.parseInt(idVocabulary) } }).ModelUVMapChannel();
    },
    WorkflowStep: async (parent: Parent, _: Args, context: Context): Promise<WorkflowStep[] | null> => {
        const { idVocabulary } = parent;
        const { prisma } = context;

        return prisma.vocabulary.findOne({ where: { idVocabulary: Number.parseInt(idVocabulary) } }).WorkflowStep();
    }
};

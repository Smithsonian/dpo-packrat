 ...
 */
import { fetchUnitForSubjectID, fetchAssetForSubjectID, fetchGeoLocationSubjectID, fetchItemsForSubjectID } from '../../../../../db';
...
const Subject = {
    ...
    items: async (parent: Parent, _: Args, context: Context): Promise<Item[] | null> => {
        ...
        return resolveItemsBySubjectID(prisma, Number.parseInt(id));
    }
};
...
export async function resolveItemsBySubjectID(prisma: PrismaClient, subjectId: number): Promise<Item[] | null> {
    const foundItems = await fetchItemsForSubjectID(prisma, subjectId);

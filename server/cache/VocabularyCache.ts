...
export class VocabularyCache {
    ...
    private async flushInternalWorker(): Promise<boolean> {
        ...
        for (const vocabulary of vocabArray) {
            ...
            if (vocabEntryArray)
                ...
                RK.logError(RK.LogSection.eCACHE,'flush worker failed','invalid CovabularySet ID',{ idVocabularySet: vocabulary.idVocabularySet },'Vocabulary.Cache');
                continue;
            }
        }
        ...
        RK.logDebug(RK.LogSection.eCACHE,'flush vocab cache success',undefined,{ vocabCount: vocabArray.length, vocabSetCount: vocabSetArray.length },'Vocabulary.Cache');
        return true;
    }
    ...
    static async mapPhotogrammetryVariantType(variantType: string): Promise<Vocabulary | undefined> {
        ...
        switch (variant) {
            ...
            case 'tif':         eVocabID = COMMON.eVocabularyID.eCaptureDataFileVariantTypeProcessed; break;
            case 'tiff':        eVocabID = COMMON.eVocabularyID.eCaptureDataFileVariantTypeProcessed; break;
            case 'bmp':         eVocabID = COMMON.eVocabularyID.eCaptureDataFileVariantTypeProcessed; break;
            case 'png':         eVocabID = COMMON.eVocabularyID.eCaptureDataFileVariantTypeProcessed; break;
            case 'jpg':         eVocabID = COMMON.eVocabularyID.eCaptureDataFileVariantTypeProcessed; break;
            case 'jpeg':        eVocabID = COMMON.eVocabularyID.eCaptureDataFileVariantTypeProcessed; break;
            ...
            default:  {
                RK.logError(RK.LogSection.eCACHE,'map variant failed','unsupported photogrammetry variant type',{ variant },'Vocabulary.Cache');
                return undefined;
            }
        }
    ...
    static mapModelFileByExtensionID(fileName: string): COMMON.eVocabularyID | undefined {
        ...
        if(nonModelTypes.includes(extension)) {
            RK.logDebug(RK.LogSection.eCACHE,'map model extension','attempting to map known type, but not a model',{ extension },'Vocabulary.Cache');
            return undefined;
        }
        ...
        switch (extension) {
            ...
            default: {
                RK.logError(RK.LogSection.eCACHE,'map model extension failed','unsupported model file extension',{ extension },'Vocabulary.Cache');
                return undefined;
            }
        }
    }
    ...
    static mapIdentifierTypeID(identifierType: string): COMMON.eVocabularyID | undefined {
        switch (identifierType.toLowerCase()) {
            ...
            default: {
                RK.logError(RK.LogSection.eCACHE,'map identifier failed','unsupported identifier type',{ identifierType },'Vocabulary.Cache');
                return undefined;
            }
        }
    }
    ...
    static mapUnits(units: string): COMMON.eVocabularyID | undefined {
        switch (units.toLowerCase()) {
            ...
            default: {
                RK.logError(RK.LogSection.eCACHE,'map units failed','unsupported units type',{ units },'Vocabulary.Cache');
                return undefined;
            }
        }
    }

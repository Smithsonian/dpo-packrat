...
export class EdanVerifier extends V.VerifierBase {
    ...
    // TODO: move core config options into base clase
    config: EdanVerifierConfig = {
        detailedLogs: true,
        logPrefix: 'EDAN Verifier',
        fixErrors: false,
        subjectLimit: 10000,    // total number of subjects to process
        systemObjectId: -1,     // limit execution to this specific SystemObject
        writeToFile: undefined  // should we dump the output to a specific path
    };

    constructor(colInstance: COL.ICollection){
        super();
        this.ICol = colInstance;
    }
    ...
    public async verify(): Promise<boolean> {
        ...
        if (!subjects) {
            LOG.error(`${this.config.logPrefix} could not get subjects from DB`, LOG.LS.eTEST);
            return false;
        }
        if(subjects.length<=0) {
            LOG.error(`${this.config.logPrefix} no subjects found in DB`, LOG.LS.eTEST);
            return false;
        }
        if(this.config.detailedLogs) console.log(`Subjects: ${subjects.length}`);

        // loop through subjects, extract name, query from EDAN
        for(let i=0; i<subjects.length; i++) {

            // adjust our counter for # of subjects and bail when done
            if(i>=this.config.subjectLimit) break;
            ...
            if(this.config.detailedLogs) console.log('SystemObject: '+JSON.stringify(systemObject));

            // (debug) if not the desired subject id skip
            if(this.config.systemObjectId>0){
                if(systemObject.idSystemObject!==this.config.systemObjectId) {
                    if(this.config.detailedLogs) console.log('Subject skipping. IDs don not match.');
                    continue;
                }
            }

            LOG.info(`${this.config.logPrefix} processing subject: ${subject.Name}`, LOG.LS.eTEST);
            if(this.config.detailedLogs) console.log('Subject:\t'+JSON.stringify(subject,null,0)); // temp

            // get our subject's unit from Packrat DB
            const packratUnit: DBAPI.Unit | null = await this.getSubjectUnit(subject);
            if(!packratUnit) {
                LOG.error(`${this.config.logPrefix} could not find a unit for subject. skipping... (id: ${systemObject.idSystemObject} | subject: ${subject.Name})`, LOG.LS.eTEST);
                continue;
            }
            if(this.config.detailedLogs) console.log('Packrat Units: '+JSON.stringify(packratUnit,null,0));
            ...
            if(!packratIdentifiers) {
                LOG.error(`${this.config.logPrefix} could not get identifiers for subject. skipping... (id: ${systemObject.idSystemObject} | subject: ${subject.Name})`, LOG.LS.eTEST);
                continue;
            }
            if(this.config.detailedLogs) console.log('Packrat Identifiers: '+JSON.stringify(packratIdentifiers,null,0));
            ...
            if(packratIdentifiers.edan && packratIdentifiers.edan.identifier) {
                ...
                LOG.error(`${this.config.logPrefix} could not get EDAN ID for subject. source of subject is ambiguous. (id: ${systemObject.idSystemObject} | subject: ${subject.Name})`, LOG.LS.eTEST);
            }
            ...
            if(packratIdentifiers.edan?.identifier)
                ...
                LOG.error(`${this.config.logPrefix} no good options for querying EDAN for subject. skipping... (id: ${systemObject.idSystemObject} | subject: ${subject.Name})`, LOG.LS.eTEST);
                continue;
            }
            if(this.config.detailedLogs) console.log('Query: '+query);
            ...
            const results: COL.CollectionQueryResults | null = await this.ICol.queryCollection(query, 10, 0, options);
            if(!results || results.records.length<=0) {
                LOG.error(`${this.config.logPrefix} did not receive records for subject and identifier. skipping... (id: ${systemObject.idSystemObject} | subject: ${subject.Name} | query: ${query})`, LOG.LS.eTEST);
                continue;
            }
            if(this.config.detailedLogs) console.log('EDAN Results: '+JSON.stringify(results,null,0));
            ...
            if(results.records.length==1) {
                ...
                if(!unit)
                    LOG.error(`${this.config.logPrefix} no known units found for subject (id: ${systemObject.idSystemObject} | subject: ${subject.Name}) and EDAN record name (${record.name})`, LOG.LS.eTEST);
                else
                    edanUnit = unit;
                if(this.config.detailedLogs) console.log('EDAN Units: '+JSON.stringify(unit,null,0));
                ...
                if(!identifiers)
                    LOG.error(`${this.config.logPrefix} no EDAN identifiers found for subject (id: ${systemObject.idSystemObject} | subject: ${subject.Name}) and EDAN record name (${record.name})`, LOG.LS.eTEST);
                else
                    edanIdentifiers = identifiers;
                if(this.config.detailedLogs) console.log('EDAN Ids: '+JSON.stringify(identifiers,null,0));

                // get our name
                edanName = record.name;

            } else {
                LOG.error(`${this.config.logPrefix} received multiple records from EDAN when expecting 1. skipping... (id: ${systemObject.idSystemObject} | subject: ${subject.Name} | query: ${query})`, LOG.LS.eTEST);
            }
            ...
            if(packratName!=edanName) {
                LOG.error(`${this.config.logPrefix} Subject name in Packrat and EDAN are not the same (id:${systemObject.idSystemObject} | Packrat:${packratName} | EDAN:${edanName})`, LOG.LS.eTEST);
                ...
                LOG.info(`${this.config.logPrefix} name compare succeeded!`, LOG.LS.eTEST);
                let str = outputPrefix;
                str += 'success,';
                str += 'name,';
                str += ',';
            ...
            if(packratUnit && edanUnit) {
                ...
                if(!foundUnit) {
                    LOG.error(`${this.config.logPrefix} Packrat unit does not match EDAN units (id:${systemObject.idSystemObject} | subject:${subject.Name})`, LOG.LS.eTEST);
                    let str = outputPrefix;
                    str += 'error,';
                    str += 'units,';
                    ...
                    if(isDPOSubject) {
                        ...
                        if(this.config.fixErrors) {
                            ...
                            if(!replaceUnitResult) {
                                LOG.error(`${this.config.logPrefix} failed to update Packrat unit to match EDAN (id:${systemObject.idSystemObject} | subject:${subject.Name})`, LOG.LS.eTEST);
                                str += 'EDAN subject. failed to automatic update. check logs or do manually';
                            } else {
                                LOG.info(`${this.config.logPrefix} successfully updated Packrat unit to match EDAN (id:${systemObject.idSystemObject} | subject:${subject.Name})`, LOG.LS.eTEST);
                    ...
                    LOG.info(`${this.config.logPrefix} Unit compare succeeded! (id:${systemObject.idSystemObject} | subject:${subject.Name})`, LOG.LS.eTEST);
                    let str = outputPrefix;
                    str += 'success,';
                    str += 'units,';
                    str += ',';
                ...
                LOG.error(`${this.config.logPrefix} Packrat unit not found in EDAN record (id:${systemObject.idSystemObject} | subject:${subject.Name} | packrat:${JSON.stringify(packratUnit)})`, LOG.LS.eTEST);
                let str = outputPrefix;
                str += 'error,';
                str += 'units,';
                ...
                LOG.error(`${this.config.logPrefix} EDAN unit not found in Packrat (id:${systemObject.idSystemObject} | subject:${subject.Name} | EDAN:${edanUnit.Name})`, LOG.LS.eTEST);
                let str = outputPrefix;
                str += 'error,';
                str += 'units,';
                ...
                LOG.error(`${this.config.logPrefix} Packrat & EDAN units not found (id:${systemObject.idSystemObject} | subject:${subject.Name})`, LOG.LS.eTEST);
                let str = outputPrefix;
                str += 'error,';
                str += 'units,';
            ...
            if(packratIdentifiers && edanIdentifiers) {
                ...
                if(idMismatch) {
                    ...
                    LOG.error(`${this.config.logPrefix} Packrat has different identifiers than EDAN (id:${systemObject.idSystemObject} | subject:${subject.Name})`, LOG.LS.eTEST);
                    LOG.error(`${this.config.logPrefix} \tPackrat: `+strPackratIds.sort().join(','), LOG.LS.eTEST);
                    LOG.error(`${this.config.logPrefix} \t   EDAN: `+strEdanIds, LOG.LS.eTEST);
                    ...
                    if(isDPOSubject) {
                        ...
                        if(this.config.fixErrors) {
                            ...
                            if(!replaceUnitResult) {
                                LOG.error(`${this.config.logPrefix} failed to update Packrat identifiers to match EDAN (id:${systemObject.idSystemObject} | subject:${subject.Name})`, LOG.LS.eTEST);
                                str += 'EDAN subject. failed to automatic update. check logs or do manually';
                            } else {
                                LOG.info(`${this.config.logPrefix} successfully updated Packrat identifiers to match EDAN (id:${systemObject.idSystemObject} | subject:${subject.Name})`, LOG.LS.eTEST);
                    ...
                    LOG.info(`${this.config.logPrefix} Identifier compare succeeded! (id:${systemObject.idSystemObject} | subject:${subject.Name})`, LOG.LS.eTEST);
                    let str = outputPrefix;
                    str += 'success,';
                    str += 'identifiers,';
                    str += ',';

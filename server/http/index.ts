...
export class HttpServer {
    ...
    private async initializeServer(): Promise<boolean> {
        ...
        if (monitorCPU) {
            const monitor: UsageMonitor = new UsageMonitor(1000, 90, 10, monitorMem, 90, 10, monitorVerbose); // sample every second, alert if > 90% for more than 10 samples in a row, monitorVerbose -> verbose logging
            monitor.start();
        }
        return res;
    }

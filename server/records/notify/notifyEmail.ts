...
import * as SHARED from './notifyShared';
import * as UTIL from '../utils/utils';

//#region UTILS
const storeServerResponse = (response: string): { statusCode: number, message: string}[] => {
...
const verifyServerResponses = (responses): boolean => {
...
const extractErrorFromResponse = (responses): string => {
...
//#region FORMATING
export const getMessageIconBase64 = (type: SHARED.NotifyType): string => {

    // pre-converted base64 strings for each icon type
    switch(type) {
        case SHARED.NotifyType.SYSTEM_ERROR:
        ...
        case SHARED.NotifyType.SYSTEM_NOTICE:
        ...
        case SHARED.NotifyType.JOB_FAILED:
        ...
        case SHARED.NotifyType.JOB_PASSED:
        ...
        case SHARED.NotifyType.JOB_STARTED:
        ...
        case SHARED.NotifyType.SECURITY_NOTICE:
...
const formatTextBody = (type: SHARED.NotifyType, params: SHARED.NotifyPackage): string => {
    const msgPrefix: string = SHARED.getMessagePrefixByType(type);
...
const formatHtmlBody = (type: SHARED.NotifyType, params: SHARED.NotifyPackage): string => {
...
//#region PUBLIC
export const sendMessageRaw = async (type: SHARED.NotifyType, sendTo: string[], subject: string, textBody: string, htmlBody?: string): Promise<SHARED.NotifyResult> => {
    ...
    return new Promise((resolve) => {
        ...
        const client = NET.createConnection(smtpPort,smtpHost, () => {
            ...
            const base64Icon: string = getMessageIconBase64(type);
        ...
        client.on('data', (data) => {
            ...
            serverResponses.push(...storeServerResponse(response));

            // see if we have an errors in the mix
            const errors = extractErrorFromResponse(serverResponses);
        ...
        client.on('end', () => {
            ...
            if(verifyServerResponses(serverResponses)===true)
                resolve({ success: true, message: 'email sent' });
            else
                resolve({ success: false, message: 'failed to send email', data: { error: extractErrorFromResponse(serverResponses) } });
        });
...
export const sendMessage = async (notifyType: SHARED.NotifyType, params: SHARED.NotifyPackage): Promise<SHARED.NotifyResult> => {
    ...
    const textBody: string = formatTextBody(notifyType, params);
    const htmlBody: string = formatHtmlBody(notifyType, params);

    try {
        const subject: string = `[Packrat:${SHARED.getMessagePrefixByType(notifyType)}] ${params.message}`;
        return await sendMessageRaw(notifyType,params.sendTo,subject,textBody,htmlBody);
    } catch (error) {

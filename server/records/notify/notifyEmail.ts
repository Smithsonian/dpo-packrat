...
export class NotifyEmail {
    ...
    //#region TESTING
    private static randomNotifyPackage(index: number): NotifyPackage {
    ...
    public static async testEmails(numEmails: number): Promise<EmailResult> {

        const hasRateManager: boolean = (NotifyEmail.rateManager) ? (NotifyEmail.rateManager!=null) : false;
        const config: RateManagerConfig<EmailEntry> | null = NotifyEmail.rateManager?.getConfig() ?? null;
        ...
        for(let i=0; i<numEmails; ++i)
            await NotifyEmail.sendMessage(NotifyEmail.randomNotifyPackage(i));

        // close our profiler and return results
        const metrics = NotifyEmail.rateManager?.getMetrics();

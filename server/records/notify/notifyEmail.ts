...
export class NotifyEmail {
    ...
    //#region FORMATING
    private static getMessageIconBase64(type: SHARED.NotifyType): string {
    ...
    private static formatTextBody(type: SHARED.NotifyType, params: SHARED.NotifyPackage): string {
    ...
    private static formatHtmlBody(type: SHARED.NotifyType, params: SHARED.NotifyPackage): string {
    ...
    //#region PUBLIC
    public static async sendMessageRaw(type: SHARED.NotifyType, sendTo: string[], subject: string, textBody: string, htmlBody?: string): Promise<SHARED.NotifyResult> {
        ...
        return new Promise((resolve) => {
            ...
            const client = NET.createConnection(smtpPort,smtpHost, () => {
                ...
                const base64Icon: string = this.getMessageIconBase64(type);
            ...
            client.on('data', (data) => {
                ...
                serverResponses.push(...this.storeServerResponse(response));

                // see if we have an errors in the mix
                const errors = this.extractErrorFromResponse(serverResponses);
            ...
            client.on('end', () => {
                ...
                if(this.verifyServerResponses(serverResponses)===true)
                    resolve({ success: true, message: 'email sent' });
                else
                    resolve({ success: false, message: 'failed to send email', data: { error: this.extractErrorFromResponse(serverResponses) } });
            });
    ...
    public static async sendMessage(notifyType: SHARED.NotifyType, params: SHARED.NotifyPackage): Promise<SHARED.NotifyResult> {
        ...
        const textBody: string = this.formatTextBody(notifyType, params);
        const htmlBody: string = this.formatHtmlBody(notifyType, params);

        try {
            const subject: string = `[Packrat:${SHARED.getMessagePrefixByType(notifyType)}] ${params.message}`;
            return await this.sendMessageRaw(notifyType,params.sendTo,subject,textBody,htmlBody);
        } catch (error) {

...
export class Project extends DBC.DBObject<ProjectBase> implements ProjectBase, SystemObjectBased {
    ...
    /**
     * Fetches Projects where the given user is explicitly authorized via UserAuthorization.
     ...
     */
    static async fetchWithUserAuthorization(idUser: number): Promise<Project[]> {
        if (!idUser)
            return [];
        try {
            return DBC.CopyArray<ProjectBase, Project>(

...
export class UserAuthorization extends DBC.DBObject<UserAuthorizationBase> implements UserAuthorizationBase {
     ...
     * For restricted projects this is the sole access path; for unrestricted projects
     * this provides additive access beyond unit membership.
     */
     ...
     * For unrestricted projects: users authorized via unit membership OR explicit project listing (additive).
     * For restricted projects: users explicitly authorized on the project only.
     */
        ...
        try {
            return await DBC.DBConnection.prisma.$queryRaw<AuthSummaryRow[]>`
                ...
                LEFT JOIN (
                    ...
                    (P.isRestricted = 0 AND (AuthUsers.authIdUnit = PU.idUnit OR AuthUsers.authIdProject = P.idProject))

...
type ComputeModelInfoResult = {
    ...
    assetVersionRoughMetal?: DBAPI.AssetVersion | undefined;
...
export class WorkflowEngine implements WF.IWorkflowEngine {
    ...
    async generateDownloads(idScene: number, workflowParams: WF.WorkflowParameters): Promise<WF.WorkflowCreateResult> {
        ...
        // A retired scene must not generate derivatives (its assets are excluded from publishing and
        // from the basename pre-flight, which would otherwise silently pass on an empty set).
        if(sceneSO.Retired) {
            RK.logError(RK.LogSection.eWF,'generate downloads blocked','scene is retired',{ idScene },'Workflow.Engine');
        ...
        const SORoughMetal: DBAPI.SystemObject| null = CSIR.assetVersionRoughMetal ? await CSIR.assetVersionRoughMetal.fetchSystemObject() : null;
        if (SORoughMetal)
            idSystemObject.push(SORoughMetal.idSystemObject);
        ...
        const jobParamSIGenerateDownloads: WFP.WorkflowJobParameters =
            new WFP.WorkflowJobParameters(COMMON.eVocabularyID.eJobJobTypeCookSIGenerateDownloads,
                new COOK.JobCookSIGenerateDownloadsParameters(CSIR.idScene, CSIR.idModel, CSIR.assetVersionGeometry.FileName,
                    CSIR.assetSVX.FileName, CSIR.assetVersionDiffuse?.FileName, CSIR.assetVersionRoughMetal?.FileName,
                    CSIR.assetVersionMTL?.FileName, sceneBaseName, CSIR.units, undefined, parameterHelper ));
    ...
    async generateScene(idModel: number, idScene: number | null, workflowParams: WF.WorkflowParameters): Promise<WF.WorkflowCreateResult> {
        ...
        // Note: a model may carry RETIRED child scenes (failed/obsolete) — those must be ignored, not
        // blocked. The reuse-vs-create decision on active scenes is made in JobCookSIVoyagerScene.
        //#endregion
        ...
        const SORoughMetal: DBAPI.SystemObject| null = CMIR.assetVersionRoughMetal ? await CMIR.assetVersionRoughMetal.fetchSystemObject() : null;
        if (SORoughMetal)
            idSystemObjects.push(SORoughMetal.idSystemObject);
        ...
        // if we still don't have a scene, try the single ACTIVE (non-retired) one from the master model.
        // Retired scenes (failed/superseded runs) are ignored — none means a new scene is created.
        if(!scene) {
            const childScenes: DBAPI.Scene[] | null = await DBAPI.Scene.fetchChildrenScenes(idModel);
            const activeChildScenes: DBAPI.Scene[] = [];
            for (const s of childScenes ?? []) {
                const sSO: DBAPI.SystemObject | null = await DBAPI.SystemObject.fetchFromSceneID(s.idScene);
                if(sSO && !sSO.Retired)
                    activeChildScenes.push(s);
            }
            if(activeChildScenes.length === 0)
                RK.logWarning(RK.LogSection.eWF,'generate scene','no active children scenes found; a new scene will be created',{ idModel },'Workflow.Engine');
            else {
                if(activeChildScenes.length > 1)
                    RK.logDebug(RK.LogSection.eWF,'generate scene',`retrieved ${activeChildScenes.length} active scenes for model`,{ idModel },'Workflow.Engine');
                scene = activeChildScenes[0];
            }
        }

        // Pre-flight basename guard (parity with Generate Downloads): when reusing an existing scene,
        ...
        const jobParamSIVoyagerScene: WFP.WorkflowJobParameters =
            new WFP.WorkflowJobParameters(COMMON.eVocabularyID.eJobJobTypeCookSIVoyagerScene,
                new COOK.JobCookSIVoyagerSceneParameters(
                    ...
                    CMIR.assetVersionRoughMetal?.FileName,    // sourceRoughMetalMapFile
                ...
                if (workflowParams.parameters.skipSceneGenerate) {
                    ...
                    const jobParamSIVoyagerScene: WFP.WorkflowJobParameters =
                        new WFP.WorkflowJobParameters(COMMON.eVocabularyID.eJobJobTypeCookSIVoyagerScene,
                            ...
                            CMIR.assetVersionDiffuse?.FileName, CMIR.assetVersionRoughMetal?.FileName,
                            sceneBaseName + '.svx.json', undefined, sceneBaseName)); //, optimalPlacement));
                    ...
                    const jobParamSIGenerateDownloads: WFP.WorkflowJobParameters =
                        new WFP.WorkflowJobParameters(COMMON.eVocabularyID.eJobJobTypeCookSIGenerateDownloads,
                            new COOK.JobCookSIGenerateDownloadsParameters(SO.idScene, CMIR.idModel, CMIR.assetVersionGeometry.FileName,
                                sceneAssetVersion.FileName, CMIR.assetVersionDiffuse?.FileName, CMIR.assetVersionRoughMetal?.FileName,
                                CMIR.assetVersionMTL?.FileName, modelBaseName));
        ...
        if(generateDownloads===true) {
            ...
            const jobParamSIGenerateDownloads: WFP.WorkflowJobParameters =
                new WFP.WorkflowJobParameters(COMMON.eVocabularyID.eJobJobTypeCookSIGenerateDownloads,
                    new COOK.JobCookSIGenerateDownloadsParameters(CSIR.idScene, CSIR.idModel, CSIR.assetVersionGeometry.FileName,
                        CSIR.assetSVX.FileName, CSIR.assetVersionDiffuse?.FileName, CSIR.assetVersionRoughMetal?.FileName,
                        CSIR.assetVersionMTL?.FileName, sceneBaseName, undefined, undefined, parameterHelper ));
    ...
    /**
     * Shared pre-flight basename guard, keyed by Cook recipe. Predicts the filenames the recipe will
     * produce for `sceneBaseName` and blocks when the scene already holds an asset carrying one of that
     * recipe's suffixes under a DIFFERENT basename — the state a model/scene rename produces, where a
     * re-run would orphan the existing set. Used by Generate Downloads ('si-generate-downloads'),
     * Generate Scene ('si-voyager-scene'), and available to the scene ingest path. The identical rule
    ...
    static async verifySceneAssetBasenameConsistency(idScene: number, sceneBaseName: string, recipe: COOK.CookRecipeKey): Promise<H.IOResults & { detail?: string }> {
        ...
        const prediction: COOK.CookOutputPrediction = COOK.predictCookOutput(recipe, sceneBaseName);
        const offenders: COOK.BasenameOffender[] = COOK.findBasenameOffenders(recipe, prediction.filenames, existingFilenames);

        if (offenders.length === 0)
            return { success: true };

        // Full expected→existing mapping goes to the log (searchable by the request's trace id shown in
        // the toast); the returned message stays short enough to read in a toast.
        const detail: string = offenders.map(o => `${o.expected} → ${o.actual}`).join('; ');
        RK.logError(RK.LogSection.eWF,'basename guard blocked',
            'existing scene assets have a different basename than the current model (rename detected)',
            { idScene, sceneBaseName, recipe, mismatches: detail }, 'Workflow.Engine');
        const n: number = offenders.length;
        return {
            success: false,
    ...
    private async computeModelInfo(idModel: number, idSystemObjectModel: number): Promise<ComputeModelInfoResult> {
        ...
        const vRoughness: DBAPI.Vocabulary | undefined = await CACHE.VocabularyCache.vocabularyByEnum(COMMON.eVocabularyID.eModelMaterialChannelMaterialTypeRoughness);
        const vMetalness: DBAPI.Vocabulary | undefined = await CACHE.VocabularyCache.vocabularyByEnum(COMMON.eVocabularyID.eModelMaterialChannelMaterialTypeMetalness);
        if (!vMaster || !vDiffuse) {
        ...
        let idAssetRoughMetal: number | null | undefined = undefined;
        ...
        let assetVersionRoughMetal: DBAPI.AssetVersion | undefined = undefined;
        ...
        if (modelConstellation.ModelMaterialChannels) {
            for (const MMC of modelConstellation.ModelMaterialChannels) {
                const isDiffuse = MMC.idVMaterialType === vDiffuse.idVocabulary;
                const isRoughness = vRoughness && MMC.idVMaterialType === vRoughness.idVocabulary;
                const isMetalness = vMetalness && MMC.idVMaterialType === vMetalness.idVocabulary;

                // Handle diffuse channel (existing logic)
                if (isDiffuse) {
                ...
                // Handle roughness OR metalness channel (they share the same image)
                if ((isRoughness || isMetalness) && idAssetRoughMetal === undefined) {
                    if (MMC.UVMapEmbedded) // stored in geometry file
                        idAssetRoughMetal = null;
                    else if (MMC.idModelMaterialUVMap) {
                        let newAssetId: number | undefined = undefined;
                        if (modelConstellation.ModelMaterialUVMaps) {
                            for (const MMUV of modelConstellation.ModelMaterialUVMaps) {
                                if (MMUV.idModelMaterialUVMap == MMC.idModelMaterialUVMap) {
                                    newAssetId = MMUV.idAsset;
                                    break;
                                }
                            }
                        }
                        if (newAssetId)
                            idAssetRoughMetal = newAssetId;
                    }
                } else if ((isRoughness || isMetalness) && idAssetRoughMetal !== undefined) {
                    // Check if roughness and metalness reference different assets
                    let newAssetId: number | undefined = undefined;
                    if (MMC.idModelMaterialUVMap && modelConstellation.ModelMaterialUVMaps) {
                        for (const MMUV of modelConstellation.ModelMaterialUVMaps) {
                            if (MMUV.idModelMaterialUVMap == MMC.idModelMaterialUVMap) {
                                newAssetId = MMUV.idAsset;
                                break;
                    ...
                    if (newAssetId && idAssetRoughMetal && newAssetId !== idAssetRoughMetal)
                        RK.logWarning(RK.LogSection.eWF,'compute model info',
                            'roughness and metalness channels reference different assets',
                            { idModel, existingAsset: idAssetRoughMetal, newAsset: newAssetId },
                            'Workflow.Engine');
                }
            }
        }
        ...
        for (const modelAsset of modelConstellation.ModelAssets) {
            ...
            if (idAssetRoughMetal === modelAsset.Asset.idAsset)
                assetVersionRoughMetal = modelAsset.AssetVersion;
        ...
        // Log warning if roughness/metallic asset was referenced but couldn't be resolved
        if (idAssetRoughMetal && idAssetRoughMetal > 0 && !assetVersionRoughMetal)
            RK.logWarning(RK.LogSection.eWF,'compute model info',
                'roughness/metallic asset referenced in channel but version not found',
                { idModel, idAssetRoughMetal },'Workflow.Engine');
        ...
        const retValue = { exitEarly: false, idModel, idSystemObjectModel, assetVersionGeometry, assetVersionDiffuse, assetVersionRoughMetal, assetVersionMTL, units };
    ...
    private async computeSceneInfo(idScene: number, idSystemObjectScene: number): Promise<ComputeSceneInfoResult> {
        ...
        const retValue = { exitEarly: false, idScene, idModel: CMIR.idModel, idSystemObjectScene,
            ...
            assetVersionRoughMetal: CMIR.assetVersionRoughMetal, assetVersionMTL: CMIR.assetVersionMTL, scene, licenseResolver, units: CMIR.units };
    ...
    static async calcOptimalPlacement (CMIR: ComputeModelInfoResult): Promise<boolean> {
        // this helps determine if decimation adjusts each new vertex to
        // its ideal position after reduction. Good for micro CT scan data
        // but can introduce errors for other model types.

        // if not an STL file ignore special treatment
        if(CMIR.assetVersionGeometry?.FileName.toLowerCase().endsWith('.stl')===false)
            return true;

        // return false
        // TODO: look for pattern in bounding box and unit size
        const modelObjects: DBAPI.ModelObject[] | null = await DBAPI.ModelObject.fetchFromModel(CMIR?.idModel ?? -1);
        if(!modelObjects || modelObjects.length<=0)
            return true;

        const modelObject = modelObjects[0];
        const validateBBoxSize = (p1: number[], p2: number[]): { valid: boolean, size: number[] } => {
            const precision = 'f32';           // MeshLab likely uses float32 internally
            const safety = 8;                     // bigger => stricter risk flagging
            const EPS = precision === 'f32' ? Math.pow(2, -23) : Math.pow(2, -52);

            const check = (a: number, b: number) => {
                if(Number.isNaN(a) || Number.isNaN(b))
                    return { delta: 0, risk: true };

                const delta = Math.abs(b - a);
                const ulp = (Math.max(Math.abs(a), Math.abs(b)) || 1) * EPS; // ~ rounding quantum near this magnitude
                return { delta, risk: delta <= safety * ulp };
            };

            const x = check(p1[0], p2[0]);
            const y = check(p1[1], p2[1]);
            const z = check(p1[2], p2[2]);

            return {
                size: [x.delta, y.delta, z.delta],
                valid: !(x.risk || y.risk || z.risk)
            };
        };
        const { valid, size } = validateBBoxSize(
